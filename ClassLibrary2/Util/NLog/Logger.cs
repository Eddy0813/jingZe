...
namespace JingZeServer.Util
{
    public class Logger
    {
        ...
        public void Process(Log log, string fileName = "")
        {
            var level = LogLevel.Info;
            if (log.Level == LevelType.Trace)
                level = LogLevel.Trace;
            else if (log.Level == LevelType.Debug)
                level = LogLevel.Debug;
            else if (log.Level == LevelType.Info)
                level = LogLevel.Info;
            else if (log.Level == LevelType.Warn)
                level = LogLevel.Warn;
            else if (log.Level == LevelType.Error)
                level = LogLevel.Error;
            else if (log.Level == LevelType.Fatal)
            ...
            ei.Properties["FileName"] = fileName;
