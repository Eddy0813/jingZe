...
using JingZeServer.DataUtil;
using System.Linq.Expressions;
using System.Reflection;
using Z.EntityFramework.Plus;

namespace JingZeUtil
{
    public class DbBase
    {
    ...
    public class DbBase<T> : DbBase where T : class, new()
    {
        ...
        public int Delete1(Expression<Func<T, bool>> whereLambda)
        {
            var entitiesToDelete = MasterDb.Set<T>().Where(whereLambda);
            MasterDb.Set<T>().RemoveRange(entitiesToDelete);
            int count = MasterDb.SaveChanges();
        ...
        public void Updaten(T model)
        {
            // 检查实体是否已经被跟踪
            var entry = MasterDb.Entry(model);
        ...
        public void Updates(T existingModel, T newModel)
        {
            // 将现有实体的状态设置为已删除
            MasterDb.Entry(existingModel).State = EntityState.Deleted;

            // 将新实体添加到上下文中
            MasterDb.Set<T>().Add(newModel);

            // 保存更改到数据库
            MasterDb.SaveChanges();
        }
        public T CloneEntity(T originalEntity)
        {
        ...
        public int UpdateByPlusmysql(Expression<Func<T, bool>> whereLambda, Expression<Func<T, T>> updateExpression, bool isOpenTran = false)
        {
            MasterDb.Configuration.EnsureTransactionsForFunctionsAndCommands = isOpenTran;

            // 获取需要更新的实体集合
            var entitiesToUpdate = MasterDb.Set<T>().Where(whereLambda).ToList();
            ...
            foreach (var entity in entitiesToUpdate)
            {
                ...
                // 将实体标记为已修改
                MasterDb.Entry(entity).CurrentValues.SetValues(updatedEntity);
            }

            // 提交更改到数据库
            return MasterDb.SaveChanges();
        }
        public void UpdateByPlusmysqln(Expression<Func<T, bool>> whereLambda, Action<T> updateAction, bool isOpenTran = false)
        {
            MasterDb.Configuration.EnsureTransactionsForFunctionsAndCommands = isOpenTran;
            ...
            foreach (var entity in entitiesToUpdate)
            {
                ...
                // 将实体标记为已修改（这一步可能不再需要，具体根据需要进行调整）
        ...
        public IQueryable<T> GetProductionStatistics(
    ...
    DbMainSlave dms = DbMainSlave.Slave, Expression<Func<T, string>> ordering = null)
        ...
        public List<TDto> SelectToListdate<TDto>(Expression<Func<T, bool>> whereLambda, Expression<Func<T, TDto>> selector, Expression<Func<TDto, DateTime>> dateField)
        {
            var query = whereLambda == null ?
                SlaveDb.Set<T>() :
                SlaveDb.Set<T>().Where(whereLambda);
            ...
            // 在排序中使用传入的日期字段
            var sortedResult = result.OrderByDescending(dateField).Take(10).ToList();
        ...
        public class ProductionData
        {
            ...
            public int HourlyProductionCount { get; set; }
        ...
        //public IQueryable<produ> GetAllProdu(bool isNoTracking = false)
        //{
        //    var query = SlaveDb.Set<produ>().AsQueryable();

        //    // Left join
        //    var joinedQuery = from a in query
        //                      join b in SlaveDb.Set<produstatu>() on a.procode equals b.procode into joinedData
        //                      from b in joinedData.DefaultIfEmpty()
        //                      select default(produ);
        //                      {
        //                          procode = a.procode,
        //                          Id = a.Id,
        //                          tickets = a.tickets,
        ...
        //    // Apply NoTracking if needed
        //    joinedQuery = isNoTracking ? joinedQuery.AsNoTracking() : joinedQuery;

        //    return joinedQuery;
        //}


        //public IQueryable<produ> GetAllprodu2(bool isNoTracking = false)
        //{
        //    var query = SlaveDb.Set<produ>().AsQueryable();
